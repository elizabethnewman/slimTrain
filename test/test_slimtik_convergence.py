...
c = A @ w0 + 1e-2 * torch.randn(A.shape[0])


# true solution
# rhs = torch.cat((c.reshape(-1), torch.zeros(A.shape[1])))
# aI = math.sqrt(num_batch * Lambda) * torch.eye(A.shape[1])
# w_true2, *_ = torch.lstsq(rhs.reshape(-1, 1), torch.cat((A, aI)))
# w_true2 = w_true2[:A.shape[1]]
u, s, v = torch.svd(A)
w_true = v @ torch.diag(s / (s ** 2 + num_batch * Lambda)) @ u.t() @ c
