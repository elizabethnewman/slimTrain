...
from autoencoder.utils import set_default_arguments_slimtrain, set_filename_slimtrain
...
# parse arguments
parser = set_default_arguments_slimtrain()
args = parser.parse_args()

filename, details = set_filename_slimtrain(args)
now = datetime.now()
...
# build network
net = MNISTAutoencoderSlimTik(width_enc=args.width_enc, width_dec=args.width_dec, intrinsic_dim=args.intrinsic_dim,
                              bias=True,
...
# optimizer
optimizer = optim.Adam(net.parameters(), weight_decay=args.alpha1, lr=args.lr)
...
if args.save:
    ...
    opt_val_loss_net.clear_()
    opt_val_loss_net.to('cpu')
    opt_val_loss_net.to_('cpu')
