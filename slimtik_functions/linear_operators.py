...
class LinearOperator:
    ...
    def __init__(self, M, alpha=1.0):
        ...
        if M is None:
            ...
            self.shape_operator = tuple(M.shape)
...
class IdentityMatrix(LinearOperator):
    def __init__(self, in_features, alpha=1.0):
        super(IdentityMatrix, self).__init__(M=None, alpha=alpha)
...
class DenseMatrix(LinearOperator):
    ...
    def __init__(self, M, alpha=1.0):
        super(DenseMatrix, self).__init__(M=M, alpha=alpha)
        self.shape_in = (M.shape[1],)
        self.shape_out = (M.shape[0],)

    def A(self, x):
        return self.alpha * self.M @ x

    def AT(self, x):
        return self.alpha * self.M.t() @ x
...
class AffineOperator(LinearOperator):
    ...
    def __init__(self, M, bias=False, alpha=1.0):
        super(AffineOperator, self).__init__(M=M, alpha=alpha)
...
class Convolution2D(LinearOperator):
    ...
    def __init__(self, M, in_channels, out_channels, kernel_size,
        ...
        super(Convolution2D, self).__init__(M=M, alpha=alpha)
        ...
        H_out = floor((M.shape[2] + 2 * padding[0] - dilation[0] * (kernel_size[0] - 1) - 1) / stride[0] + 1)
        W_out = floor((M.shape[3] + 2 * padding[1] - dilation[1] * (kernel_size[1] - 1) - 1) / stride[1] + 1)
    ...
    def A(self, x):
        ...
        z = F.conv2d(self.alpha * self.M, x.reshape(self.shape_in), bias=b, stride=self.stride,
    ...
    def AT(self, x):
        ...
        z = F.conv2d(self.alpha * self.M.permute(1, 0, 2, 3), x.permute(1, 0, 2, 3), bias=None,
    ...
    def numel_out(self):
        return self.M.shape[0] * prod(self.shape_out)
...
class ConvolutionTranspose2D(LinearOperator):
    ...
    def __init__(self, M, in_channels, out_channels, kernel_size,
        ...
        super(ConvolutionTranspose2D, self).__init__(M=M, alpha=alpha)
        ...
        H_out = (M.shape[2] - 1) * stride[0] - 2 * padding[0] + dilation[0] * (kernel_size[0] - 1) \
                + output_padding[0] + 1
        W_out = (M.shape[3] - 1) * stride[1] - 2 * padding[1] + dilation[1] * (kernel_size[1] - 1) \
                + output_padding[1] + 1
    ...
    def A(self, x):
        ...
        z = F.conv_transpose2d(self.alpha * self.M, x.reshape(self.shape_in), bias=b,
                               stride=self.stride,
    ...
    def AT(self, x):
        ...
        z = F.conv2d(x.permute(1, 0, 2, 3), self.alpha * self.M.permute(1, 0, 2, 3), bias=None,
